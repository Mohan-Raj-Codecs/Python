...
class PowBox(bsBomb.Bomb):
	...
	def setPowText(self, color=(1, 1, 0.4)):
		...
		self._powText = bs.newNode('text',
									  owner=self.node,
									  attrs={'text':u'\ue043Subscribe YSB GROUP On Youtube\ue043',
											 'inWorld':True,
											 'shadow':1.0,
											 'flatness':1.0,
											 'color':color,
	...
	def handleMessage(self, m):
		if isinstance(m, bs.HitMessage):
			...
			if m.hitType == 'punch':
				...
				flashColor = (random.random(),random.random(),random.random())
				light = bs.newNode("light",
								   attrs={'position':punchPos,
										  'radius':0.25+hurtiness*0.25,
										  'intensity':0.5*(1.0+1.0*hurtiness),
