    ...
    def handleMessage(self, msg):
        ...
        if isinstance(msg, bs.PickedUpMessage):
            ...
            if (msg.powerupType == 'tripleBombs'):
                ...
                bsUtils.PopupText('Energy Shield', color=(
            ...
            elif msg.powerupType == 'shower':
                tex = bs.Powerup.getFactory().texshower
                bsUtils.PopupText('Have Bath in Impact', color=((0+random.random()*1.0), (0+random.random(
                )*1.0), (0+random.random()*1.0)), scale=1.6, position=self.node.position).autoRetain()
                sd=self.node.position
                lst=list(sd)
                def dropBomb():
                    b = bs.Bomb(position=(lst[0],lst[1]+1,lst[2]), velocity=(0,25,0),bombType='impact', blastRadius=2.0, owner=self.node).autoRetain()
                    lst[1]+=0
                bs.gameTimer(100, bs.Call(dropBomb))
                bs.gameTimer(300, bs.Call(dropBomb))
                bs.gameTimer(500, bs.Call(dropBomb))
                bs.gameTimer(700, bs.Call(dropBomb))
                bs.gameTimer(900, bs.Call(dropBomb))
                bs.gameTimer(1000, bs.Call(dropBomb))
                
            elif msg.powerupType == 'speedy':
                tex = bs.Powerup.getFactory().texspeedy
                bsUtils.PopupText('Speed Boots', color=((0+random.random()*1.0), (0+random.random(
                )*1.0), (0+random.random()*1.0)), scale=1.6, position=self.node.position).autoRetain()
                ...
                if self.powerupsExpire:
                    ...
                    self.node.miniBillboard3StartTime = t
                    self.node.miniBillboard3EndTime = t+gPowerupWearOffTime
                    self._speedyWearOffFlashTimer = bs.Timer(
                        gPowerupWearOffTime-1000, bs.WeakCall(self._speedyWearOffFlash))
                    self._speedyWearOffTimer = bs.Timer(
                        gPowerupWearOffTime, bs.WeakCall(self._speedyWearOff))

            elif msg.powerupType == 'bbomb':
                tex = bs.Powerup.getFactory().texbbomb
                bsUtils.PopupText("FRY", color=(
                    0, 1.0, 1.0), scale=1.6, position=self.node.position).autoRetain()
                ...
                if self.powerupsExpire:
                    ...
                    self.node.miniBillboard3StartTime = t
                    self.node.miniBillboard3EndTime = t+gPowerupWearOffTime
                    self._bbombWearOffFlashTimer = bs.Timer(
                        gPowerupWearOffTime-1000, bs.WeakCall(self._bbombWearOffFlash))
                    self._bbombWearOffTimer = bs.Timer(
                        gPowerupWearOffTime, bs.WeakCall(self._bbombWearOff))

            elif msg.powerupType == 'virus':
                factory = self.getFactory()
                tex = bs.Powerup.getFactory().texvirus
                self.node.handleMessage("knockout", 2000)
                bsUtils.PopupText("Little Zzzzz", color=(
                    1.0, 2.0, 0), scale=1.6, position=self.node.position).autoRetain()
                if self._cursed:
                    self._cursed = False
                ...
                bsUtils.PopupText("Bot Army", color=(
                    0, 1.0, 1.0), scale=1.6, position=self.node.position).autoRetain()
                self._bots = bs.BotSet()
                sd = self.node.position
                lst = list(sd)
                bs.gameTimer(1000, bs.Call(self._bots.spawnBot, bs.BunnyBot, pos=(
                    lst[0]-1, lst[1]+1, lst[2]), spawnTime=100))
                bs.gameTimer(1500, bs.Call(self._bots.spawnBot, bs.NinjaBot, pos=(
                    lst[0], lst[1]+1, lst[2]), spawnTime=200))
                bs.gameTimer(2000, bs.Call(self._bots.spawnBot, bs.ToughGuyBot, pos=(
                    lst[0]+1, lst[1]+1, lst[2]), spawnTime=300))
                bs.gameTimer(2500, bs.Call(self._bots.spawnBot, bs.NinjaBot, pos=(
                    lst[0]+2, lst[1]+1, lst[2]), spawnTime=400))

            elif msg.powerupType == 'smario':
                tex = bs.Powerup.getFactory().texsmario
                bsUtils.PopupText("Super Mario", color=(
                    0, 1.0, 1.0), scale=1.6, position=self.node.position).autoRetain()
                ...
                delay=0
                for i in range(1,200):  #100(delays)x200 - 20 Seconds i.e Power gonna Expire
                    bs.gameTimer(delay, bs.Call(colr))
                    delay+=100
...
##                    random.random()*2, random.random()*2, random.random()*2)
                ...
                if self.powerupsExpire:
                    ...
                    self.node.miniBillboard3StartTime = t
                    self.node.miniBillboard3EndTime = t+gPowerupWearOffTime
                    self._smarioWearOffFlashTimer = bs.Timer(
                        gPowerupWearOffTime-1000, bs.WeakCall(self._smarioWearOffFlash))
                    self._smarioWearOffTimer = bs.Timer(
                        gPowerupWearOffTime, bs.WeakCall(self._smarioWearOff))
                ...
                def my():
                    bs.emitBGDynamics(
                        position=self.node.position, velocity=velocity, count=200, scale=1.0, chunkType='metal')
                    bs.emitBGDynamics(position=self.node.position, velocity=velocity,
                                      count=20, scale=1.0, chunkType='spark', emitType='stickers')
                    bs.emitBGDynamics(position=self.node.position, velocity=velocity,
                                      count=100, scale=1.0, spread=1.5, chunkType='spark')
                ...
                bsUtils.PopupText("Super Punch", color=(
                    0, 1.0, 1.0), scale=1.6, position=self.node.position).autoRetain()
                self._punchCooldown = 0
                # trust me u don't want to allow him to finish the whole team alone
                self._punchPowerScale = 4.0
                if self.powerupsExpire:
                    self.node.miniBillboard3Texture = tex
                    t = bs.getGameTime()
                    self.node.miniBillboard3StartTime = t
                    self.node.miniBillboard3EndTime = t+gPowerupWearOffTime
                    self._spunchWearOffFlashTimer = bs.Timer(
                        gPowerupWearOffTime-1000, bs.WeakCall(self._spunchWearOffFlash))
                    self._spunchWearOffTimer = bs.Timer(
                        gPowerupWearOffTime, bs.WeakCall(self._spunchWearOff))
            elif msg.powerupType == 'curse':
                bsUtils.PopupText("Dead", color=(
                    1.0, 1.0, 1.0), scale=1.6, position=self.node.position).autoRetain()
                self.curse()
            elif (msg.powerupType == 'iceBombs'):
                self.bombType = 'ice'
                bsUtils.PopupText("Ice Bomb ", color=(
                    1.1, 1.0, 2.0), scale=1.6, position=self.node.position).autoRetain()
                tex = self._getBombTypeTex()
                self._flashBillboard(tex)
                if self.powerupsExpire:
                    self.node.miniBillboard2Texture = tex
                ...
                bsUtils.PopupText("Health", color=(
                    1.0, 2.0, 0), scale=1.6, position=self.node.position).autoRetain()
                if self._cursed:
                    self._cursed = False
                    # remove cursed material
            ...
            if msg.hitType == 'punch':
                ...
                if damage >= 1 and damage < 99:
                    bsUtils.PopupText("Ultra Noob", color=((0+random.random()*1.0), (0+random.random(
                    )*1.0), (0+random.random()*1.0)), scale=1.6, position=self.node.position).autoRetain()

                elif damage >= 100 and damage < 200:
                    bsUtils.PopupText("Noob", color=((0+random.random()*1.0), (0+random.random()*1.0),
                                                     (0+random.random()*1.0)), scale=1.6, position=self.node.position).autoRetain()

                elif damage >= 201 and damage < 300:
                    bsUtils.PopupText("Chaapa", color=((0+random.random()*1.0), (0+random.random(
                    )*1.0), (0+random.random()*1.0)), scale=1.6, position=self.node.position).autoRetain()

                elif damage >= 301 and damage < 400:
                    bsUtils.PopupText("Nooooo", color=((0+random.random()*1.0), (0+random.random(
                    )*1.0), (0+random.random()*1.0)), scale=1.6, position=self.node.position).autoRetain()

                elif damage >= 401 and damage < 500:
                    bsUtils.PopupText("Good", color=((0+random.random()*1.0), (0+random.random()*1.0),
                                                     (0+random.random()*1.0)), scale=1.6, position=self.node.position).autoRetain()

                elif damage >= 501 and damage < 600:
                    bsUtils.PopupText("Better", color=((0+random.random()*1.0), (0+random.random(
                    )*1.0), (0+random.random()*1.0)), scale=1.6, position=self.node.position).autoRetain()

                elif damage >= 601 and damage < 700:
                    bsUtils.PopupText("Fuck!", color=((0+random.random()*1.0), (0+random.random(
                    )*1.0), (0+random.random()*1.0)), scale=1.6, position=self.node.position).autoRetain()

                elif damage >= 701 and damage < 800:
                    bsUtils.PopupText("Vera Level", color=((0+random.random()*1.0), (0+random.random(
                    )*1.0), (0+random.random()*1.0)), scale=1.6, position=self.node.position).autoRetain()

                elif damage >= 801 and damage < 900:
                    bsUtils.PopupText("verithanam", color=((0+random.random()*1.0), (0+random.random(
                    )*1.0), (0+random.random()*1.0)), scale=1.6, position=self.node.position).autoRetain()

                elif damage >= 901 and damage < 1000:
                    bsUtils.PopupText("Pro!!", color=((0+random.random()*1.0), (0+random.random(
                    )*1.0), (0+random.random()*1.0)), scale=1.6, position=self.node.position).autoRetain()
                    bs.playSound(bs.getSound("actionHeroDeath"))
                elif damage > 1000:
                    bsUtils.PopupText("Blasted", color=((0+random.random()*1.0), (0+random.random(
                    )*1.0), (0+random.random()*1.0)), scale=1.6, position=self.node.position).autoRetain()
                if damage > 350:
...
class PlayerSpaz(Spaz):
    ...
    def connectControlsToPlayer(self, enableJump=True, enablePunch=True,
        ...
        if self._connectedToPlayer is not None:
            if player != self._connectedToPlayer:
                player.resetInput()
...
class BotSet(object):
    ...
    def clear(self):
        ...
        if activity is None or activity.isFinalized():
            return
