...
class Spaz(bs.Actor):
    ...
    def handleMessage(self, msg):
        ...
        if isinstance(msg, bs.PickedUpMessage):
            ...
            if (msg.powerupType == 'tripleBombs'):
                bsUtils.PopupText("Triple", color=(
                    3.0, 1.0, 2.0), scale=1.6, position=self.node.position).autoRetain()
                ...
                bsUtils.PopupText("Landmines", color=(
                    1.0, 0, 0), scale=1.6, position=self.node.position).autoRetain()
                self.setLandMineCount(min(self.landMineCount+3, 3))
            elif msg.powerupType == 'impactBombs':
                self.bombType = 'impact'
                bsUtils.PopupText("Impact Bombs", color=(
                    0, 0, 1.0), scale=1.6, position=self.node.position).autoRetain()
                ...
                bsUtils.PopupText(" StickyBombs ", color=(
                    0, 1.0, 0), scale=1.6, position=self.node.position).autoRetain()
                ...
                bsUtils.PopupText("Boxer", color=(
                    1.0, 0, 1.0), scale=1.6, position=self.node.position).autoRetain()
                ...
                alist = ['mohanRaj', 'mohan', 'Mohan', 'mohanraj', 'mohan raj',
                    'Mohan raj', 'Mohanraj', 'MohanRaj', 'Mohan Raj']
                if self.node.name in alist:
                    bsUtils.PopupText('Pro Dont Need This!!', color=(
                        0, 1.0, 1.0), scale=1.6, position=self.node.position).autoRetain()
                    bs.ScreenMessage("Faked by this", color=(
                        0, 0, 1))  # Created to Fake Powerup
                else:
                    bsUtils.PopupText('Noob U Need This!!', color=(
                        0, 1.0, 1.0), scale=1.6, position=self.node.position).autoRetain()
                    factory = self.getFactory()
            ...
            elif msg.powerupType == 'sman':
                tex = bs.Powerup.getFactory().texsman
                self._hasBoxingGloves = True
                tex = bs.Powerup.getFactory().texsman
                self._hasBoxingGloves = True
                velocity = (1, 3, 1)
                ...
                def my():
                    bs.emitBGDynamics(position=self.node.position, velocity=velocity,
                                      count=200, scale=1.0, chunkType='slime' or 'slime');
                    bs.emitBGDynamics(position=self.node.position, velocity=velocity,
                                      count=20, scale=1.0, chunkType='spark', emitType='stickers');
                    bs.emitBGDynamics(position=self.node.position, velocity=velocity,
                                      count=100, scale=1.0, spread=1.5, chunkType='spark');
                bs.gameTimer(100, my)
                ...
                explosion.color = (2.0, 0.06, 0)
                bsUtils.PopupText("SuperMan", color=(
                    2.0, 0, 1.0), scale=1.6, position=self.node.position).autoRetain()
                self._flashBillboard(tex)
                self.equipBoxingGloves()
                # Shield
                factory = self.getFactory()
                ...
                if self.powerupsExpire:
                    ...
                    self.node.miniBillboard3StartTime = t
                    self.node.miniBillboard3EndTime = t+gPowerupWearOffTime
                    self._smanWearOffFlashTimer = bs.Timer(
                        gPowerupWearOffTime-1000, bs.WeakCall(self._smanWearOffFlash))
                    self._smanWearOffTimer = bs.Timer(
                        gPowerupWearOffTime, bs.WeakCall(self._smanWearOff))

            elif msg.powerupType == 'slm':
            	tex = bs.Powerup.getFactory().texslm
            	timet = bs.getSharedObject('globals').slowMotion
            	if timet == True:
                    bsUtils.PopupText("Faster", color=(
                        0, 1.0, 1.0), scale=1.6, position=self.node.position).autoRetain()
                    bs.getSharedObject('globals').slowMotion = False
                else:
                    bsUtils.PopupText("Slower", color=(
                        0, 1.0, 1.0), scale=1.6, position=self.node.position).autoRetain()
                    bs.getSharedObject('globals').slowMotion = True

             elif msg.powerupType == 'speedy':
            	tex = bs.Powerup.getFactory().texspeedy
                bsUtils.PopupText('Speed Boots',color=((0+random.random()*1.0),(0+random.random()*1.0),(0+random.random()*1.0)),scale=1.6,position=self.node.position).autoRetain()
                # connectControlsToPlayer(self.node,enableFly=True)
            	...
            	if self.powerupsExpire:
                    ...
                    self.node.miniBillboard3StartTime = t
                    self.node.miniBillboard3EndTime = t+gPowerupWearOffTime
                    self._speedyWearOffFlashTimer = bs.Timer(gPowerupWearOffTime-1000,bs.WeakCall(self._speedyWearOffFlash))
                    self._speedyWearOffTimer = bs.Timer(gPowerupWearOffTime,bs.WeakCall(self._speedyWearOff))


            elif msg.powerupType == 'bbomb':
            	tex = bs.Powerup.getFactory().texbbomb
            	bsUtils.PopupText("FRY",color=(0,1.0,1.0),scale=1.6,position=self.node.position).autoRetain()
                s=bs.Bomb.getFactory()
                s.bombModel = bs.getModel('egg')
                s.stickyBombModel = bs.getModel('egg')
                s.impactBombModel = bs.getModel('egg')
                self.blastRadius = 8.0
                if self.powerupsExpire:
                    self.node.miniBillboard3Texture = tex
                    t = bs.getGameTime()
                    self.node.miniBillboard3StartTime = t
                    self.node.miniBillboard3EndTime = t+gPowerupWearOffTime
                    self._bbombWearOffFlashTimer = bs.Timer(gPowerupWearOffTime-1000,bs.WeakCall(self._bbombWearOffFlash))
                    self._bbombWearOffTimer = bs.Timer(gPowerupWearOffTime,bs.WeakCall(self._bbombWearOff))
            elif msg.powerupType == 'virus':
                factory = self.getFactory()
            	tex = bs.Powerup.getFactory().texvirus
                self.node.handleMessage("knockout",9999)
                bsUtils.PopupText("Zzzzz",color=(1.0,2.0,0),scale=1.6,position=self.node.position).autoRetain()
                if self._cursed:
                    self._cursed = False
                ...
                self._flashBillboard(bs.Powerup.getFactory().texvirus)
                self.node.hurt = 0
                self._lastHitTime = None
                self._numTimesHit = 0
            elif msg.powerupType == 'boti':
            	tex = bs.Powerup.getFactory().texboti
            	bsUtils.PopupText("Bot Army",color=(0,1.0,1.0),scale=1.6,position=self.node.position).autoRetain()
                self._bots = bs.BotSet()
                sd=self.node.position
                lst=list(sd)
		bs.gameTimer(1000,bs.Call(self._bots.spawnBot,bs.BunnyBot,pos=(lst[0]-1,lst[1]+1,lst[2]),spawnTime=100))
            	bs.gameTimer(1500,bs.Call(self._bots.spawnBot,bs.NinjaBot,pos=(lst[0],lst[1]+1,lst[2]),spawnTime=200))
		bs.gameTimer(2000,bs.Call(self._bots.spawnBot,bs.ToughGuyBot,pos=(lst[0]+1,lst[1]+1,lst[2]),spawnTime=300))
		bs.gameTimer(2500,bs.Call(self._bots.spawnBot,bs.NinjaBot,pos=(lst[0]+2,lst[1]+1,lst[2]),spawnTime=400))
		bs.gameTimer(3000,bs.Call(self._bots.spawnBot,bs.PirateBotNoTimeLimit,pos=(lst[0]+1,lst[1]+1,lst[2]),spawnTime=1000))
                               
            
            elif msg.powerupType == 'smario':
            	tex = bs.Powerup.getFactory().texsmario
            	bsUtils.PopupText("Super Mario",color=(0,1.0,1.0),scale=1.6,position=self.node.position).autoRetain()
                bs.playSound(bs.getSound('wrestler2'))
                for i in range(1):
                    global nocolo,nohigh
                    nocolo= self.node.color
                    nohigh=self.node.highlight
                # def colr():
                 #   self.node.color = (random.random()*2,random.random()*2,random.random()*2)
                  #  self.node.highlight = (random.random()*2,random.random()*2,random.random()*2)
                self.node.invincible = True
                self.node.color = (random.random()*2,random.random()*2,random.random()*2)
                self.node.highlight = (random.random()*2,random.random()*2,random.random()*2)
                # self.colorSet = bs.Timer(100,bs.Call(colr),repeat=True)
                if self.powerupsExpire:
                    self.node.miniBillboard3Texture = tex
                    t = bs.getGameTime()
                    self.node.miniBillboard3StartTime = t
                    self.node.miniBillboard3EndTime = t+gPowerupWearOffTime
                    self._smarioWearOffFlashTimer = bs.Timer(gPowerupWearOffTime-1000,bs.WeakCall(self._smarioWearOffFlash))
                    self._smarioWearOffTimer = bs.Timer(gPowerupWearOffTime,bs.WeakCall(self._smarioWearOff))
                ...
                radiusw=5.2
            	def my():
                    bs.emitBGDynamics(position=self.node.position,velocity=velocity,count=200,scale=1.0,chunkType='metal');
                    bs.emitBGDynamics(position=self.node.position,velocity=velocity,count=20,scale=1.0,chunkType='spark',emitType='stickers');
                    bs.emitBGDynamics(position=self.node.position,velocity=velocity,count=100,scale=1.0,spread=1.5,chunkType='spark');
                bs.gameTimer(100,my)
                explosion = bs.newNode("explosion",
                                      attrs={'position':self.node.position,
                                      'velocity':(velocity[0],max(-1.0,velocity[1]),velocity[2]),
                ...
                explosion.color = (0.05,0.05,1.0)
            	bsUtils.PopupText("Super Punch",color=(0,1.0,1.0),scale=1.6,position=self.node.position).autoRetain()
            	self._punchCooldown=0
                self._punchPowerScale = 4.0 #trust me u don't want to allow him to finish the whole team alone
                if self.powerupsExpire:
                    self.node.miniBillboard3Texture = tex
                    t = bs.getGameTime()
                    self.node.miniBillboard3StartTime = t
                    self.node.miniBillboard3EndTime = t+gPowerupWearOffTime
                    self._spunchWearOffFlashTimer = bs.Timer(gPowerupWearOffTime-1000,bs.WeakCall(self._spunchWearOffFlash))
                    self._spunchWearOffTimer = bs.Timer(gPowerupWearOffTime,bs.WeakCall(self._spunchWearOff))
            elif msg.powerupType == 'curse':
                bsUtils.PopupText("Dead",color=(1.0,1.0,1.0),scale=1.6,position=self.node.position).autoRetain()
                self.curse()
            elif (msg.powerupType == 'iceBombs'):
                self.bombType = 'ice'
                bsUtils.PopupText("Ice Bomb ",color=(1.1,1.0,2.0),scale=1.6,position=self.node.position).autoRetain()
                tex = self._getBombTypeTex()
                self._flashBillboard(tex)
                if self.powerupsExpire:
                    self.node.miniBillboard2Texture = tex
                ...
                bsUtils.PopupText("Health",color=(1.0,2.0,0),scale=1.6,position=self.node.position).autoRetain()
                if self._cursed:
                    self._cursed = False
                    # remove cursed material
                    factory = self.getFactory()
            ...
            if msg.hitType == 'punch':
                ...
                if damage >= 1 and damage < 99:
                     bsUtils.PopupText("Ultra Noob",color=((0+random.random()*1.0),(0+random.random()*1.0),(0+random.random()*1.0)),scale=1.6,position=self.node.position).autoRetain()                        

                elif damage >= 100 and damage < 200 :
                     bsUtils.PopupText("Noob",color=((0+random.random()*1.0),(0+random.random()*1.0),(0+random.random()*1.0)),scale=1.6,position=self.node.position).autoRetain()

                elif damage >= 201 and damage < 300 :
                     bsUtils.PopupText("Chaapa",color=((0+random.random()*1.0),(0+random.random()*1.0),(0+random.random()*1.0)),scale=1.6,position=self.node.position).autoRetain()

                elif damage >= 301 and damage < 400 :
                     bsUtils.PopupText("Nooooo",color=((0+random.random()*1.0),(0+random.random()*1.0),(0+random.random()*1.0)),scale=1.6,position=self.node.position).autoRetain()

                elif damage >= 401 and damage < 500 :
                     bsUtils.PopupText("Good",color=((0+random.random()*1.0),(0+random.random()*1.0),(0+random.random()*1.0)),scale=1.6,position=self.node.position).autoRetain()

                elif damage >= 501 and damage < 600 :
                     bsUtils.PopupText("Better",color=((0+random.random()*1.0),(0+random.random()*1.0),(0+random.random()*1.0)),scale=1.6,position=self.node.position).autoRetain()

                elif damage >= 601 and damage < 700 :
                     bsUtils.PopupText("Fuck!",color=((0+random.random()*1.0),(0+random.random()*1.0),(0+random.random()*1.0)),scale=1.6,position=self.node.position).autoRetain()

                elif damage >= 701 and damage < 800 :
                     bsUtils.PopupText("Vera Level",color=((0+random.random()*1.0),(0+random.random()*1.0),(0+random.random()*1.0)),scale=1.6,position=self.node.position).autoRetain()

                elif damage >= 801 and damage < 900 :
                     bsUtils.PopupText("verithanam",color=((0+random.random()*1.0),(0+random.random()*1.0),(0+random.random()*1.0)),scale=1.6,position=self.node.position).autoRetain()   

                elif damage >= 901 and damage < 1000 :
                     bsUtils.PopupText("Pro!!",color=((0+random.random()*1.0),(0+random.random()*1.0),(0+random.random()*1.0)),scale=1.6,position=self.node.position).autoRetain()   
                     bs.playSound(bs.getSound("actionHeroDeath"))
                elif damage > 1000 :
                    bsUtils.PopupText("Blasted",color=((0+random.random()*1.0),(0+random.random()*1.0),(0+random.random()*1.0)),scale=1.6,position=self.node.position).autoRetain()
                if damage > 350:
    ...
    def _smanWearOffFlash(self):
        if self.node.exists():
            self.node.boxingGlovesFlashing = 1
            self.node.billboardTexture = bs.Powerup.getFactory().texsman
            self.node.billboardOpacity = 1.0
            self.node.billboardCrossOut = True

    def _smanWearOff(self):
        if self._demoMode: # preserve old behavior
            self._punchPowerScale = gBasePunchPowerScale
            self._punchCooldown = gBasePunchCooldown
        else:
    ...
    def _speedyWearOffFlash(self):
          if self.node.exists():
                self.node.billboardTexture = bs.Powerup.getFactory().texspeedy
                self.node.billboardOpacity = 1.0
                self.node.billboardCrossOut = True
            
    def _speedyWearOff(self):
          factory = self.getFactory()
          self.node.hockey = False
          if self.node.exists():
                self.node.billboardOpacity = 0.0
                
    def _bbombWearOffFlash(self):
        if self.node.exists():
            self.node.billboardTexture = bs.Powerup.getFactory().texbbomb
            self.node.billboardOpacity = 1.0
            self.node.billboardCrossOut = True
            
    def _bbombWearOff(self):
        factory = self.getFactory()
        s=bs.Bomb.getFactory()
        s.bombModel = bs.getModel('ninjaHead')
        s.stickyBombModel = bs.getModel('bonesHead')
        s.impactBombModel = bs.getModel('impactBomb')
        self.blastRadius = 2.0
        if self.node.exists():
            self.node.billboardOpacity = 0.0
            
    def _smarioWearOffFlash(self):
        if self.node.exists():
            self.node.billboardTexture = bs.Powerup.getFactory().texsmario
            self.node.billboardOpacity = 1.0
            self.node.billboardCrossOut = True
            
    def _smarioWearOff(self):
        factory = self.getFactory()
        self.node.invincible = False
        self.node.color = nocolo 
        self.node.highlight = nohigh
        if self.node.exists():
            self.node.billboardOpacity = 0.0
             
    def _spunchWearOffFlash(self):
        if self.node.exists():
            self.node.billboardTexture = bs.Powerup.getFactory().texspunch
            self.node.billboardOpacity = 1.0
            self.node.billboardCrossOut = True
            
    def _spunchWearOff(self):
        factory = self.getFactory()
        self._punchPowerScale = factory.punchPowerScale
        self._punchCooldown = factory.punchCooldown
        if self.node.exists():
...
t.iconTexture = "actionHeroIconColorMask"
...
t.defaultColor = (random.random()*2,random.random()*2,random.random()*2)
