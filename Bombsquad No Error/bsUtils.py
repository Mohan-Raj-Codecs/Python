...
def createUserSystemScripts():
    ...
    try:
        ...
        if exc.errno == errno.EEXIST and os.path.isdir(path + "_tmp"):
            pass
        else:
            raise
...
def isBrowserLikelyAvailable():
    ...
    if platform == 'android' and subplatform == 'ouya':
        return False
    ...
    for i, gamespec in enumerate(copy.deepcopy(playlist)):
        ...
        if type(gamespec['type']) in [str, unicode]:
            try:
                ...
                if removeUnOwned and gameClass in unOwnedGameTypes:
                    continue
                if addResolvedType:
                    gamespec['resolvedType'] = gameClass
...
def _setLanguage(language, printChange=True, storeToConfig=True):
    ...
    try:
        ...
        if language != 'English':
            reload(l)  # helpful for iterating
    except Exception:
    ...
    if l is not None:
        languages.append(l)
    if language != 'English':
        languages.append(__import__('bsLanguageEnglish'))
    languages.reverse()
    lFull = AttrDict()
    for l in languages:
...
if bs.getEnvironment()['platform'] == 'android':
    ...
    class ITunesThread(threading.Thread):
        ...
        def setVolume(self, volume):
            ...
            # don't touch itunes..
            if self._currentPlaylist is None:
                return
...
def setMusicPlayMode(mode, forceRestart=False):
    ...
    if oldMode != gMusicMode or forceRestart:
        ...
        if mode == 'regular':
            _playMusic(gMusicTypes['regular'])
    ...
    try:
        if entry is None:
            ...
            raise Exception("invalid soundtrack entry: " + str(entry) +
                            " (type " + str(type(entry)) + ")")
        if _isSoundtrackEntryTypeSupported(entryType):
            return entryType
        else:
            raise Exception("invalid soundtrack entry:" + str(entry))
    except Exception as e:
...
def _playMusic(musicType,
    ...
    with bs.Context('UI'):
        ...
        if continuous and gMusicTypes[mode] == musicType:
            return
...
def getPlayerProfileColors(profileName, profiles=None):
    ...
    bsConfig = bs.getConfig()
    if profiles is None:
        profiles = bsConfig['Player Profiles']
...
def printLiveObjectWarnings(when, ignoreSession=None, ignoreActivity=None):
    ...
    for obj in gc.get_objects():
        if isinstance(obj, bs.Actor):
            actors.append(obj)
        elif isinstance(obj, bs.Session):
            sessions.append(obj)
        elif isinstance(obj, bs.Activity):
            activities.append(obj)
    # complain about any remaining sessions
    for session in sessions:
        if session is ignoreSession:
            continue
...
class ControlsHelpOverlay(bsGame.Actor):
    ...
    def __init__(self,
        ...
        if showTitle:
            self._nodes.append(self._titleText)
    ...
    def _update(self):
        ...
        if len(jumpButtonNames) == 1:
            t = list(jumpButtonNames)[0]
        else:
            t = ''
...
class Image(bsGame.Actor):
    ...
    def __init__(self,
        ...
        if modelOpaque is not None:
            self.node.modelOpaque = modelOpaque
        if modelTransparent is not None:
            self.node.modelTransparent = modelTransparent
        if tint2Color is not None:
            self.node.tint2Color = tint2Color
...
def _callAfterAd(c):
    ...
    if show:
        ...
        if _gAdAmt is None:
            ...
            if launchCount < minLC:
                lcAmt = 0.0
            elif launchCount > maxLC:
                lcAmt = 1.0
            else:
                lcAmt = (float(launchCount) - minLC) / (maxLC - minLC)
