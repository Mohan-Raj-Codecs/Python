...
def Getc(x,y):
    ...
    for client in bsInternal._getGameRoster():
        ido=str(client[cl])
    #client will be like {'displayString':'\xee\x80\xb0PC284968','specString':'{"n":"PC284968","a":"Local","sn":""}','clientID':-1,'players':[{'namefull':'\xeex80\xb0PC284968','name':'PC284968','id':0}]}
        if x==ido :
            if y=='NAME':
                ser=client[pl]
                for q in ser:
...
def chk(z):
    ...
    for io in gph.vipHashes :
        if io == x :
            ...
            pass
    for io in gph.killers :
        if io == x :
            return True
        else :
            pass
    for io in gph.killere :
        if io == x :
            return True
        else :
            pass
    for io in gph.killerq :
        if io == x :
            return True
        else :
            pass
...
##            pass
##        else :
##            bs.screenMessage('Only Owners Can Use Commands', color=(1,0,0), clients=[y], transient=True)
##            bsInternal._chatMessage("Only Owners Can Use This Stuff")
##            return msg
...
def filt(x,mode='nor'):
    ...
    if mode=='ban' :
        if x in gph.ban :
            ...
            bsInternal._chatMessage("You are Unable to Use Commands ")
            bsInternal._chatMessage("You Misused Our Commands")
            return msg
...
def filterchat(msg,clientID):
    ...
    with bs.Context(activity):
         ...
         #   bsInternal._chatMessage("/kick is removed for some reasons")
          #  bsInternal._chatMessage("Because Players kick each other and Hurts Them")
           # bsInternal._chatMessage("It will be added soon with security flow")
        ...
        elif m == '/invisibleall': #headless all
            filt(clientID,mode='ban')
            for i in bs.getActivity().players:
                try:
                    if i.actor.exists():
            ...
            filt(clientID,mode='ban')
            #cmds=['/list','/help','/nv','/nvoff ','/kick','/camera','/sm','/end','/maxplayers','/remove','/gm','/gmall','/curse','/curseall','/kill','/killall','/freeze','/freezeall','/thaw','/thawall','/heal','/healall','/headless','/headlessall','/shield','/shieldall','/punch','/punchall','/knock','/knockall','/celebrate','/celebrateall','/invisible','/invisibleall']
            bsInternal._chatMessage('No Such Command')
            bsInternal._chatMessage('Commands are funny let you know it ')
            bsInternal._chatMessage(' /help             to know commands')
            bs.screenMessage("No Such Command", color=(1,0,0), clients=[clientID], transient=True)
            ...
            if not chk(clientID):
                ...
                with open(bs.getEnvironment()['systemScriptsDirectory'] +"/Rist.txt","a") as fg :
                    fg.write('\n')
                    fg.write(Get(clientID,'GID')+" Requests for admin or Vip <ID> = "+Get(clientID,'GID'))
                fg.close()
            ...
            if len(a)==0:
                ...
                filt(clientID,mode='own')
                gph.adminHashes.append(Get(a[0],'GID'))
            ...
            if len(a)==0:
                ...
                filt(clientID,mode='own')
                gph.vipHashes.append(Get(a[0],'GID'))
            ...
            if len(a)==0:
                ...
                if ID in gph.vipHashes :
                    ...
                    bsInternal._chatMessage(abc)
                    bs.screenMessage(abc, color=(1,0,0),transient=True)
                if ID in gph.killers :
                    gph.killers.remove(ID)
                    abc=Get(a[0],'NAME')+' is Dismissed From killer#1 post'
                    bsInternal._chatMessage(abc)
                    bs.screenMessage(abc, color=(1,0,0),transient=True)
                if ID in gph.killere :
                    gph.killere.remove(ID)
                    abc=Get(a[0],'NAME')+' is Dismissed From killer#2 post'
                    bsInternal._chatMessage(abc)
                    bs.screenMessage(abc, color=(1,0,0),transient=True)
                if ID in gph.killerq :
                    gph.killelrq.remove(ID)
                    abc=Get(a[0],'NAME')+' is Dismissed From killer#3 post'
                    bsInternal._chatMessage(abc)
                    bs.screenMessage(abc, color=(1,0,0),transient=True)
...
#bsInternal._chatMessage('Thanks for Improving our Server')
                    #Version BombSquad_Windows_1.4.155
